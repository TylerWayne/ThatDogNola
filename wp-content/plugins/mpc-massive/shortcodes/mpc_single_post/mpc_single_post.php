	...
	class MPC_Single_Post {
		...
		function __construct() {
			...
			if ( function_exists( 'vc_lean_map' ) ) {
				vc_lean_map( 'mpc_single_post', array( $this, 'shortcode_map' ) );
			} else {
			...
			/* Autocomplete */
			add_filter( 'vc_autocomplete_mpc_single_post_item_id_callback', 'vc_include_field_search', 10, 1 );
			add_filter( 'vc_autocomplete_mpc_single_post_item_id_render', 'vc_include_field_render', 10, 1 );
		...
		function enqueue_shortcode_scripts() {
			wp_enqueue_style( 'mpc_single_post-css', MPC_MASSIVE_URL . '/shortcodes/mpc_single_post/css/mpc_single_post.css', array(), MPC_MASSIVE_VERSION );
			wp_enqueue_script( 'mpc_single_post-js', MPC_MASSIVE_URL . '/shortcodes/mpc_single_post/js/mpc_single_post' . MPC_MASSIVE_MIN . '.js', array( 'jquery' ), MPC_MASSIVE_VERSION );
		}
		...
		function build_query( $atts ) {
			...
			$args[ 'post__in' ] = explode( ', ', $atts[ 'item_id' ] );
		...
		function get_thumbnail() {
			...
			if( $thumbnail ) {
				$image_atts = ' style="background-image: url(' . esc_attr( esc_url( $thumbnail[ 0 ] ) ). ');"';
			} else {
				$image_atts = false;
			}

			return apply_filters( 'ma/single_post/get/thumbnail', $image_atts, 'get_thumbnail' );
		}

		/* Build Thumbnail */
		function build_thumbnail( $layout, $disable ) {
			if ( $disable == '' ) {
				$thumbnail        = $this->get_thumbnail();
				$thumbnail_layout = $this->thumbnail_layout( $layout );

				$return = $thumbnail != false ? '<div class="mpc-post__thumbnail"' . $thumbnail . '>' . $thumbnail_layout . '</div>' : $this->get_placeholder( $thumbnail_layout );
		...
		function shortcode_layout( $style ) {
			...
			$layouts = array(
				'style_1' => array( 'thumbnail', 'overlay', 'section_begin', 'title', 'meta', 'section_end' ),
				'style_2' => array( 'thumbnail', 'overlay', 'section_begin', 'title', 'description', 'readmore', 'meta', 'section_end' ),
				'style_3' => array( 'thumbnail', 'overlay', 'section_begin', 'date', 'wrapper_begin', 'title', 'meta', 'wrapper_end', 'description', 'readmore', 'section_end' ),
				'style_4' => array( 'thumbnail', 'section_begin', 'wrapper_begin', 'title', 'meta', 'wrapper_end', 'overlay', 'section_end' ),
				'style_5' => array( 'thumbnail', 'overlay', 'section_begin', 'title', 'description', 'meta', 'readmore', 'section_end' ),
				'style_6' => array( 'thumbnail', 'overlay', 'section_begin', 'wrapper_begin', 'date', 'wrapper_end', 'title', 'meta', 'section_end' ),
				'style_7' => array( 'thumbnail', 'section_begin', 'title', 'meta', 'overlay', 'section_end' ),
				'style_8' => array( 'thumbnail', 'overlay' ),
		...
		function pagination_content( $atts ) {
			...
			while( $this->query->have_posts() ) {
				...
				$this->parts[ 'thumbnail' ] = $this->build_thumbnail( $atts[ 'layout' ], $atts[ 'disable_thumbnail' ] );

				$return .= $this->before;

				$return .= '<div onclick="" class="mpc-post">';
		...
		function shortcode_template( $atts, $content = null, $shortcode = null, $parent_css = null ) {
			...
			/* Build Query */
			if ( ! is_object( $this->query ) && $atts[ 'item_id' ] == '' ) {
				return '';
			}
			...
			while( $this->query->have_posts() ) {
				...
				if( ! $this->is_wrapped ) {
					$return .= '<div' . $css_id . ' class="mpc-single-post' . esc_attr( $this->classes ) .'">';
				}
		...
		function shortcode_styles( $styles, $parent_css ) {
			...
			// Thumb Height
			if( $styles[ 'thumb_height' ] ) {
				$style .= $css_selector . ' .mpc-post__thumbnail {';
					$style .=  'height: ' . $styles[ 'thumb_height' ] . ';';
				$style .= '}';

				$style .= $css_selector . ' .mpc-post__thumbnail + .mpc-post__overlay {';
					$style .=  'height: ' . $styles[ 'thumb_height' ] . ';';
				$style .= '}';
			}
...
//				'icon'        => MPC_MASSIVE_URL . '/assets/images/icons/mpc-single-display.png',
