...
if ( ! class_exists( 'MPC_WC_Product' ) ) {
	class MPC_WC_Product {
		...
		function get_thumbnail() {
			...
			return apply_filters( 'ma/product/get/thumbnail', $image_atts, 'get_thumbnail' );
		}
		...
		function get_lightbox_src() {
			$thumbnail_id = get_post_thumbnail_id( get_the_ID() );
			$thumbnail    = apply_filters( 'ma/product/lightbox_src', wp_get_attachment_image_src( $thumbnail_id, 'full' )[ 0 ], 'lightbox_src' );
		...
		function pagination_content( $atts ) {
			...
			while ( $this->query->have_posts() ) {
				...
				$this->parts[ 'thumbnail' ] = $this->build_thumbnail();


				$content .= '<div class="mpc-wc-product' . $classes_effect . '">';
		...
		function shortcode_template( $atts, $content = null, $shortcode = null, $parent_css = null ) {
			...
			if ( $mpc_ma_options[ 'enabled_shortcodes' ][ 'all' ] === '0' ) {
			...
			/* Build Query */
			if ( ! is_object( $this->query ) && $atts[ 'id' ] == '' ) {
				return '';
			}
			...
			while ( $this->query->have_posts() ) {
				...
				if( !$this->is_wrapped ) {
					$return .= '<div' . $css_id . ' class="mpc-wc-product' . $this->classes . $classes_effect . '"' . $this->attributes . '>';
				} else {
					$return .= '<div class="mpc-wc-product' . $classes_effect . '">';
				}
