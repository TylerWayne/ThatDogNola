...
if ( ! class_exists( 'MPC_Carousel_Image' ) ) {
	class MPC_Carousel_Image {
		...
		function enqueue_shortcode_scripts() {
			wp_enqueue_style( 'mpc_carousel_image-css', MPC_MASSIVE_URL . '/shortcodes/mpc_carousel_image/css/mpc_carousel_image.css', array(), MPC_MASSIVE_VERSION );
			wp_enqueue_script( 'mpc_carousel_image-js', MPC_MASSIVE_URL . '/shortcodes/mpc_carousel_image/js/mpc_carousel_image' . MPC_MASSIVE_MIN . '.js', array( 'jquery' ), MPC_MASSIVE_VERSION );
		}
			...
			$return = '<div class="mpc-carousel__wrapper mpc-waypoint">';
				...
				foreach( $images as $image_id ) {
					...
					$item = '<' . $wrapper  . ' class="' . $classes_item . '" data-height="' . esc_attr( $image[ 'p_img_large' ][ 2 ] ) . '">';
						$item .= '<div onclick="" class="mpc-item mpc-transition">' . $border . $image[ 'thumbnail' ]  . $overlay . '</div>';
					$item .= '</' . $wrapper_end . '>';
...
//				'icon'        => MPC_MASSIVE_URL . '/assets/images/icons/mpc-carousel-image.png',
