...
if ( ! class_exists( 'MPC_Column' ) ) {
	class MPC_Column {
		...
		/* fb:like g:plus etc */
		function pre_parse_namespaces( $output ) {
			$output = preg_replace( '/<(([^\s\/]*):([\S]*))([>|\s])/', '<mpc__$2_$3$4', $output );
			$output = preg_replace( '/<\/(([\S]*):([\S]*))>/', '</mpc__$2_$3>', $output );
		...
		function column_output( $output, $shortcode, $atts ) {
			...
			// Convert namespace based markup
			$output = $this->pre_parse_namespaces( $output );
