...
if ( ! class_exists( 'MPC_Textblock' ) ) {
	class MPC_Textblock {
		...
		function enqueue_shortcode_scripts() {
			wp_enqueue_style( 'mpc_textblock-css', MPC_MASSIVE_URL . '/shortcodes/mpc_textblock/css/mpc_textblock.css', array(), MPC_MASSIVE_VERSION );
			wp_enqueue_script( 'mpc_textblock-js', MPC_MASSIVE_URL . '/shortcodes/mpc_textblock/js/mpc_textblock' . MPC_MASSIVE_MIN . '.js', array( 'jquery' ), MPC_MASSIVE_VERSION );
		}
		...
		function shortcode_styles( $styles ) {
			...
			// Add '%'
			$margin = 100 - $styles[ 'content_width' ];
			$styles[ 'content_width' ] = $styles[ 'content_width' ] != '' ? $styles[ 'content_width' ] . ( is_numeric( $styles[ 'content_width' ] ) ? '%' : '' ) : '';

			$inner_styles = array();
			if ( $styles[ 'inline' ] == '' && $styles[ 'content_width' ] != '' ) {
				$inner_styles[] = 'max-width:' . $styles[ 'content_width' ] . ';';

				if( $styles[ 'font_align' ] == 'right' ) {
					$inner_styles[] = 'margin-left:' . $margin . '%;';
				} else if( !$styles[ 'font_align' ] == 'left' ) {
					$inner_styles[] = 'margin-left:' . (int) $margin * .5 . '%;margin-right:' . (int) $margin * .5 . '%;';
				}
			}
