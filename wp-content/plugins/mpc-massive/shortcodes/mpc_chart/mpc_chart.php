...
if ( ! class_exists( 'MPC_Chart' ) ) {
	class MPC_Chart {
		...
		function enqueue_shortcode_scripts() {
			wp_enqueue_style( 'mpc_chart-css', MPC_MASSIVE_URL . '/shortcodes/mpc_chart/css/mpc_chart.css', array(), MPC_MASSIVE_VERSION );
			wp_enqueue_script( 'mpc_chart-js', MPC_MASSIVE_URL . '/shortcodes/mpc_chart/js/mpc_chart' . MPC_MASSIVE_MIN . '.js', array( 'jquery' ), MPC_MASSIVE_VERSION );
		}
		...
		function shortcode_map() {
			...
			return array(
				...
				'base'        => 'mpc_chart',
				'class'       => '',
//				'icon'        => MPC_MASSIVE_URL . '/assets/images/icons/mpc-charts.png',
		...
		function add_background_options( &$options, $atts, $prefix ) {
			...
			if ( $atts[ $prefix . '_background_type' ] == 'color' && $atts[ $prefix . '_background_color' ] != '' ) {
				...
				if ( isset( $background_image[ 0 ] ) ) {
					...
					$options[ $prefix ][ 'image' ] = MPC_MASSIVE_URL . '/assets/images/mpc-image-placeholder.png';
				}
