...
if ( ! class_exists( 'MPC_Navigation' ) ) {
	class MPC_Navigation {
		...
		function enqueue_shortcode_scripts() {
			wp_enqueue_style( 'mpc_navigation-css', MPC_MASSIVE_URL . '/shortcodes/mpc_navigation/css/mpc_navigation.css', array(), MPC_MASSIVE_VERSION );
			wp_enqueue_script( 'mpc_navigation-js', MPC_MASSIVE_URL . '/shortcodes/mpc_navigation/js/mpc_navigation' . MPC_MASSIVE_MIN . '.js', array( 'jquery' ), MPC_MASSIVE_VERSION );
		}
		...
		function shortcode_template( $name, $content = null, $css_id = '', $type = '', $carousel = '' ) {
			...
			$parts            = array();
			$parts[ 'begin' ] = '<div class="mpc-navigation' . $classes . ' mpc-nav-preset--' . esc_attr( $atts[ 'preset' ] ) . '" data-slider="' . $css_id . '">';
			$parts[ 'end' ]   = '</div>';
