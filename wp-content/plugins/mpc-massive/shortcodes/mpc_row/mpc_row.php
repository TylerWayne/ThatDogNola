...
if ( ! class_exists( 'MPC_Row' ) ) {
	class MPC_Row {
		...
		function cache_atts( $class, $tag = '', $atts = array() ) {
			...
			$cache_id = 'mpc-atts-cache-' . MPC_Helper::generateRandomString();
		...
		function verify_node( DOMNode $row ) { // ToDo: Find more themes related classes
			...
			foreach( $valid_classes as $valid_class ) {
				if( !$is_valid && preg_match( '/(?<![\S])' . $valid_class . '/mU', $row_class ) == 1 ) {
					return $row;
				}
			}
			...
			foreach( $nodes as $node ) {
				...
				foreach( $valid_classes as $valid_class ) {
					if( !$is_valid && preg_match( '/(?<![\S])' . $valid_class . '/mU', $node_class ) == 1 ) {
						return $node;
					}
				}
			}
		...
		function detect_closing_node( &$output, &$prepend, &$append ) {
			// Find first opening tag and return if at 0/1
			preg_match( '/(<(?!\/))/im', $output, $position, PREG_OFFSET_CAPTURE );
			if( !isset( $position[ 0 ][ 1 ] ) || $position[ 0 ][ 1 ] <= 1 ) return;

			// Strip and save the closing elements
			if( !isset( $position[ 0 ][ 0 ] ) && !isset( $position[ 0 ][ 1 ] ) ) return;
			$prepend = substr( $output, 0, $position[ 0 ][ 1 ] );
			$output  = substr_replace( $output, '', 0, $position[ 0 ][ 1 ] );

			// Detect last closing element and strip content after it
			preg_match_all( '/(<\/(.*)>)/im', $output, $position, PREG_OFFSET_CAPTURE );
			$position = end( $position ); $position = end( $position ); // Double last index
			if( !isset(  $position[ 0 ] ) && !isset( $position[ 1 ] ) ) return;
			$position = $position[ 1 ] + strlen( '</' . $position[ 0 ] . '>' ) - strlen( $output );

			if( !isset( $position ) || $position >= 0 ) return;
			$append = substr( $output, $position );
			$output = substr_replace( $output, '', $position );
		}

		function post_parse_namespaces( $output ) {
		...
		public function row_output( $output, $shortcode, $atts ) {
			...
			$this->sh_atts[ 'data-row-id' ] = $this->css_id;
			$this->row_class();
			...
			// Detect closing nodes before row & opening nodes after row
			$output_prepend = $output_append = '';
			$this->detect_closing_node( $output, $output_prepend, $output_append );

			// Convert HTML markup inside scripts
			$output_append .= MPC_Helper::get_scripts();
			...
			// Prepend Row
			if( $this->prepend != '' ) {
				$this->prepend = MPC_Helper::encoder( $this->prepend );
			...
			// Append Row
			if( $this->append != '' ) {
				$this->append = MPC_Helper::encoder( $this->append );
			...
			// Before Row
			if( $this->before != '' ) {
				$this->before = MPC_Helper::encoder( $this->before );
				$content_html->loadHTML( $this->before );
				$nodes = $content_html->documentElement->firstChild->childNodes;

				foreach( $nodes as $node ) {
					$new_node = $this->html->importNode( $node, true );
					$row->parentNode->insertBefore( $new_node, $row );
				}
			}

			// After Row
			if( $this->after != '' ) {
				$this->after = MPC_Helper::encoder( $this->after );
				$content_html->loadHTML( $this->after );
				$nodes = $content_html->documentElement->firstChild->childNodes;

				foreach( $nodes as $node ) {
					$new_node = $this->html->importNode( $node, true );
					$row->parentNode->appendChild( $new_node );
				}
			}
			...
			$output = $this->post_parse_namespaces( $output );
			$output = $output_prepend . $output . $output_append;

			unset( $row, $content_html, $nodes, $output_prepend, $output_append );
		...
		function toggle_row() {
			if ( $this->atts[ 'toggle_enable' ] ) {
				...
				$mpc_toggle_row = '<div id="' . $this->css_id . '" class="mpc-toggle-row' . $this->classes . $mpc_stretch_classes . $mpc_font_classes . $mpc_transition_classes . '">';
