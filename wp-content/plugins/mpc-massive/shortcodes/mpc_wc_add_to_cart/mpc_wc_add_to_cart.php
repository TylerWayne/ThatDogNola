...
if ( ! class_exists( 'MPC_WC_Add_To_Cart' ) ) {
	class MPC_WC_Add_To_Cart {
		...
		function __construct() {
			...
			/* Autocomplete */
			add_filter( 'vc_autocomplete_mpc_wc_add_to_cart_item_id_callback', array( &$this, 'productIdAutocompleteSuggester' ), 10, 1 );
			add_filter( 'vc_autocomplete_mpc_wc_add_to_cart_item_id_render', array( &$this, 'productIdAutocompleteRender' ), 10, 1 );
		...
		public function productIdAutocompleteSuggester( $query ) {
			...
			$post_meta_infos = $wpdb->get_results(
				$wpdb->prepare( "SELECT a.ID AS id, a.post_title AS title, b.meta_value AS sku
					...
					WHERE ( a.post_type = 'product' OR a.post_type = 'product_variation' ) AND ( a.ID = '%d' OR b.meta_value LIKE '%%%s%%' OR a.post_title LIKE '%%%s%%' )",
			...
			if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
				foreach ( $post_meta_infos as $value ) {
					$data = array();
					$data['value'] = $value['id'];
					$data['label'] = __( 'Id', 'mpc' ) . ': ' .
						$value['id'] .
						( ( strlen( $value['title'] ) > 0 ) ? ' - ' . __( 'Title', 'mpc' ) . ': ' .
							$value['title'] : '' ) .
						( ( strlen( $value['sku'] ) > 0 ) ? ' - ' . __( 'Sku', 'mpc' ) . ': ' .
							$value['sku'] : '' );
					$results[] = $data;
				}
			}
		...
		public function productIdAutocompleteRender( $query ) {
			...
			if ( ! empty( $query ) ) {
				...
				$variation_object = new WC_Product_Variation( (int) $query );
		...
		function enqueue_shortcode_scripts() {
			wp_enqueue_style( 'mpc_wc_add_to_cart-css', MPC_MASSIVE_URL . '/shortcodes/' . $this->shortcode . '/css/' . $this->shortcode . MPC_MASSIVE_MIN . '.css', array(), MPC_MASSIVE_VERSION );
			wp_enqueue_script( 'mpc_wc_add_to_cart-js', MPC_MASSIVE_URL . '/shortcodes/' . $this->shortcode . '/js/' . $this->shortcode . MPC_MASSIVE_MIN . '.js', array( 'jquery' ), MPC_MASSIVE_VERSION );
		}
		...
		function shortcode_styles( $styles, $parent_css = '' ) {
			...
			if ( count( $inner_styles ) > 0 ) {
				$style .= $styles[ 'hover_title_type' ] == 'none' ? '[data-id="' . $css_id . '"] .mpc-atc__title-hover .mpc-atc__icon,' : '';
